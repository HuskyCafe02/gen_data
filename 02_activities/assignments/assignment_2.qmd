...
```{r}
# Your code here...
q3 <- fread("./02_activities/data/gwa.A2.fam")

#histogram
v <- c(q3$V6)
hist(v, xlab="phenotype", col="darkgreen", border="black" )

#density plot
den <-density(q3$V6)
ggplot(q3, aes(x=V6)) + geom_density(fill = "darkgreen") + labs(title = "Q3", x = "phenotype", y="density")
...
```{bash}
# Your code here...
plink2 --bfile ./02_activities/data/gwa.qc.A2 --indep-pairwise 500 50 0.05 --out ./02_activities/data/gwa.qc.A2.pruned

plink2 --bfile ./02_activities/data/gwa.qc.A2 --extract ./02_activities/data/gwa.qc.A2.pruned.prune.in --make-bed --out ./02_activities/data/gwa.qc.A2.pruned
```
...
```{bash}
# Your code here...
plink2 --bfile ./02_activities/data/gwa.qc.A2.pruned --king-cutoff 0.0884 --out ./02_activities/data/gwa.A2.king
```
...
```{bash}
# Your code here...
plink2 --bfile ./02_activities/data/gwa.qc.A2 --keep ./02_activities/data/gwa.A2.king.king.cutoff.in.id --make-bed --out ./02_activities/data/gwa.qc.A2.unrelated
```
...
```{bash}
# Your code here...
plink2 --bfile ./02_activities/data/gwa.qc.A2.unrelated --extract ./02_activities/data/gwa.qc.A2.pruned.prune.in --make-bed --out ./02_activities/data/gwa.qc.A2.unrelated.pruned

plink2 --bfile ./02_activities/data/gwa.qc.A2.unrelated.pruned --pca 20 --out ./02_activities/data/gwa.A2.unrel.PCA
```
...
```{bash}
# Your code here...
plink2 --bfile ./02_activities/data/gwa.qc.A2 --ci 0.95 --logistic --covar ./02_activities/data/gwa.A2.unrel.PCA.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc.A2.assoc
```
...
```{r}
# Your code here...
assoc <- data.table::fread("./02_activities/data/gwa.qc.A2.assoc.PHENO1.glm.linear")

assoc_add <- assoc[TEST == "ADD"]

setnames(assoc_add, c("#CHROM","POS","ID"), c("CHR","BP","SNP"))

png("./02_activities/data/manhattan.plot.A2.png", width=1200, height=800, res=150)
...
manhattan(assoc_add,
          ...
          cex.axis = 1.5,
          col = c("darkgreen", "lightslateblue"))
...
🚨 **Please review our [Assignment Submission Guide](https://github.com/UofTUofT-DSI/onboarding/blob/main/onboarding_documents/submissions.md)** 🚨 for detailed instructions on how to format, branch, and submit your work. Following these guidelines is crucial for your submissions to be evaluated correctly.
