...
#The phenotype is not continuous, therefore the answer is continuous
```
...
```         
# Your code here...
#Since all of their frequencies are below 50%, it is the same as the frequency given in part (i)
```
...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
--hardy \
--out ./02_activities/data/gwa.qc.A1.hwe

{r}
hwe <- fread("./02_activities/data/gwa.qc.A1.hwe.hardy")
head(hwe)
```
...
--extract ./02_activities/data/A1snplist.txt \
--hardy \
--out ./02_activities/data/gwa.qc.A1.3.hwe

{r}
hwe <- fread("./02_activities/data/gwa.qc.A1.3.hwe.hardy")
head(hwe)
...
# Your code here...
{bash}
plink2 --bfile ./02_activities/data/gwa.qc.A1 --freq --out ./02_activities/data/gwa.qc.freq.A1

{r}
freq <- fread("./02_activities/data/gwa.qc.freq.A1.afreq")
...
{r}
ADmodel <- lm(PHENOTYPE ~ rs1861_C, data = geno_subset)
summary(ADmodel)
...
```         
# Your answer here...
Given the extremely small p-value of less than 2.2e-16, it suggests a strong correlation between the SNPs and the phenotype.
```
...
{r}
 ...
 geno_range$predicted_prob <- predict(ADmodel, newdata = geno_range, type = "response")
...
print(p1)
ggsave("./02_activities/data/A1p1.png", plot = p1, width = 7, height = 5, dpi = 300)
```
...
# Your code here...
{r}
RECmodel <- lm(PHENOTYPE ~ rs1861_C, data = geno_subset_rec)
summary (RECmodel)
#p-value: < 2.2e-16
```
...
# Your code here...
{r}
model <- lm(PHENOTYPE ~ rs1861_C, data = geno_subset_rec)
summary(model)

geno_range <- data.frame(
   rs1861_C = seq(0, 1, length.out = 100)
 )
 geno_range$predicted_prob <- predict(RECmodel, newdata = geno_range, type = "response")
...
print(p3)
ggsave("./02_activities/data/A1p3.png", plot = p3, width = 7, height = 5, dpi = 300)
...
```         
# Your answer here...
AIC(ADmodel, RECmodel)
#The AD model has a lower AIC, therefore the Additive model is a better fit.
